...
Map::Map(MapLoader* mapLoaderObject)
{
    ...
    for (int i = 0; i < territoryCount; i++) {
        ...
        terrVector.at(i).continentID = mapLoaderObject->countryContinentIdArray[i];
        terrVector.at(i).territoryID = i;
        contVector.at(mapLoaderObject->countryContinentIdArray[i]).head = new ContinentNode{&terrVector.at(i), contVector.at(mapLoaderObject->countryContinentIdArray[i]).head};
        contVector.at(mapLoaderObject->countryContinentIdArray[i]).length++;
    }
}
...
{
    return this->countryContinentIdArray;
}
...
MapLoader::MapLoader(string filename)
{
                            ...
                            if (line[i] == ' ') {
                                ...
                                for (size_t j = i; j < line.length(); j++) {
                                    if(line[j] == ' ') {
                                        ...
                                        countryContinentIdArray[countryCount] = contIdInt;
                                        countryCount++;
                                        contIDString.clear();
                                        contIdDigits = 0;
                                        break;
    ...
    for(int i = 0; i < countryCount; i++) {
        for(int j = 0; j < borders[i].size(); j++) {
            if(i+1 != borders[i][j]) {
                inputMap->AddEdge(i+1, borders[i][j]);
            }
        }
    }
